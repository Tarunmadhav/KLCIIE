-- ============================================================================
-- KL CIIE Platform V2 — 0039: attendance QR rotation window 20s -> 100s
--
-- Recreates get_my_event_attendance_qr so unused codes rotate after 100
-- seconds instead of 20 seconds. The client countdown (QR_ROTATE_MS) must
-- match this window.
-- ============================================================================
...
begin
  ...
  -- Rotate unused codes that were issued more than 100 seconds ago so a
  ...
  where eq.event_id = p_event_id
    ...
    and now() - eq.code_issued_at >= interval '100 seconds';
